...
var parseReceiveMessageResults = function (res, receiveData) {
    ...
    if (receiveData &&
        ...
        receiveData.ReceiveMessageResponse.ReceiveMessageResult.messages)
    {
        assert(receiveData.ReceiveMessageResponse.ReceiveMessageResult.messages.length === 1, 'only one message at a time');
        message = receiveData.ReceiveMessageResponse.ReceiveMessageResult.messages[0];
    }
...
SQS.prototype.pop = function (task) {
    ...
    return q.resolve().then(_.bind(function () {
        return this._receiveMessage();
    }, this)).then(function (message) {
